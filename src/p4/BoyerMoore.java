...
public class BoyerMoore {
    ...
    /**
     * Returns the indexes of the occurrences of the pattern string
     * in the text string.
     *
     * @author Ferdinand & Thorben
     * @param  txt the text string
     * @return Iterable of the indexes of the occurrences of the pattern string
     * in the text string.
     */
    public Collection<Integer> findAll(String txt){
        Collection<Integer> occurences = new ArrayList<>();
        int m = pat.length();
        int n = txt.length();
        int skip;
        for (int i = 0; i <= n - m; i += skip) {
            ...
            // found
            if (skip == 0){
                ((ArrayList<Integer>) occurences).add(i);
                i++;
            }
        }
        return occurences;
