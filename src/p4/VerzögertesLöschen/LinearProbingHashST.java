...
public class LinearProbingHashST<Key, Value> {
    ...
    public void put(Key key, Value val) {
        ...
        if(delayed){
            ...
            if (n >= (m-deletedVals)/2) resize(2*(m-deletedVals));
        }else {
            // double table size if 50% full
            if (n >= m/2) resize(2*m);
        }
    ...
    public void deleteDelayed(Key key) {
        if (key == null) throw new IllegalArgumentException("argument to delete() is null");
        if (!contains(key)) return;
        ...
        if (n-deletedVals > 0 && n-deletedVals <= m/8) resize(m/2);
        assert check();
    }
    ...
    private boolean check() {
        ...
        if (m < 2*n) {
            ...
            return false;
        }else if(m-deletedVals < 2*n){
            System.err.println("Hash table size m = " + (m-deletedVals) + "; array size n = " + n);
            return false;
